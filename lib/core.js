...
// Read head part of a document to scan for the RDFa version, base URI, and other critical information
// No RDF statements may be emitted during this process
// For SAX streams, this must also buffer the read nodes until the head tag closes
RDFaParser.prototype.scanDocument = function scanDocument(document){
	// void
};
	...
	if(typeof setProperty=='string'){
		...
		termProperty.forEach(function(predicate){
			if(predicate.termType=='BlankNode'){
				self.warning('Ignoring RDF Statement with blanknode for predicate');
				return;
			}
